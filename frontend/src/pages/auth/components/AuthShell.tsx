...
import { LuBadgeCheck, LuKeyRound, LuShieldCheck } from 'react-icons/lu'
...
import authHeroIllustration from '@/assets/images/auth-hero.svg'
...
const AuthShell = ({
  ...
  const isTurkish = i18n.resolvedLanguage?.toLowerCase().startsWith('tr')
  const heroFallbacks = isTurkish
    ? {
        eyebrow: 'Korumali erisim',
        title: 'Guvenligi hissettiren daha temiz bir giris deneyimi.',
        subtitle: 'Giris, OTP dogrulama ve hesap kurtarma adimlari ayni tutarli akisla ilerler.',
        security: 'Korumali giris',
        otp: 'OTP dogrulama',
        recovery: 'Kurtarma hazir',
        protection: 'Rate-limit kalkani',
      }
    : {
        eyebrow: 'Protected access',
        title: 'A cleaner sign-in experience with security built in.',
        subtitle: 'Login, OTP verification, and recovery all follow the same reliable flow.',
        security: 'Protected login',
        otp: 'OTP verification',
        recovery: 'Recovery ready',
        protection: 'Rate-limit shield',
      }

  const bodyClasses = ['min-vh-100', 'd-flex', 'flex-column', 'justify-content-center', bodyClassName]
    .filter(Boolean)
    .join(' ')

  const contentClasses = [contentClassName].filter(Boolean).join(' ')
      ...
      <Row className="w-100 g-0">
          ...
          <Card className="auth-box-form border-0 mb-0">
            <div className="position-absolute top-0 end-0" style={{ width: 180 }}>
              <svg
                ...
                <g clipPath="url(#authShellClip)">
                  <mask id="authShellMaskPrimary" style={{ maskType: 'luminance' }} maskUnits="userSpaceOnUse" x={0} y={0} width={600} height={1200}>
                    <path d="M0 0L0 1200H600L600 0H0Z" fill="white" />
                  </mask>
                  <g mask="url(#authShellMaskPrimary)">
                    <path d="M537.448 166.697L569.994 170.892L550.644 189.578L537.448 166.697Z" fill="#FF4C3E" />
                  </g>
                  <mask id="authShellMaskSecondary" style={{ maskType: 'luminance' }} maskUnits="userSpaceOnUse" x={0} y={0} width={600} height={1200}>
                    <path d="M0 0L0 1200H600L600 0H0Z" fill="white" />
                  </mask>
                  <g mask="url(#authShellMaskSecondary)">
                    <path
        ...
        <Col>
          <div className="h-100 position-relative card-side-img rounded-0 overflow-hidden">
              <div className="card-img-overlay auth-overlay d-flex flex-column justify-content-between p-4 p-xl-5">
              <div className="auth-side-chip align-self-start">
                <LuShieldCheck size={18} />
                <span>{t('auth.hero.eyebrow', { defaultValue: heroFallbacks.eyebrow })}</span>
              </div>

              <div className="auth-side-content mx-auto w-100">
                <div className="auth-side-copy text-center text-xl-start">
                  <p className="auth-side-kicker mb-2">{applicationName}</p>
                  <h2 className="auth-side-title mb-3">
                    {t('auth.hero.title', { defaultValue: heroFallbacks.title })}
                  </h2>
                  <p className="auth-side-subtitle mb-0">
                    {t('auth.hero.subtitle', { defaultValue: heroFallbacks.subtitle })}
                  </p>
                </div>

                <div className="auth-side-visual position-relative">
                  <img
                    src={authHeroIllustration}
                    alt=""
                    className="auth-side-illustration img-fluid"
                  />

                  <div className="auth-side-card auth-side-card-primary">
                    <div className="auth-side-card-icon">
                      <LuBadgeCheck size={18} />
                    </div>
                    <span>{t('auth.hero.metrics.security', { defaultValue: heroFallbacks.security })}</span>
                  </div>

                  <div className="auth-side-card auth-side-card-secondary">
                    <div className="auth-side-card-icon">
                      <LuKeyRound size={18} />
                    </div>
                    <span>{t('auth.hero.metrics.otp', { defaultValue: heroFallbacks.otp })}</span>
                  </div>
                </div>

                <div className="auth-side-stat-row">
                  <div className="auth-side-stat">
                    <strong>24/7</strong>
                    <span>{t('auth.hero.metrics.recovery', { defaultValue: heroFallbacks.recovery })}</span>
                  </div>
                  <div className="auth-side-stat">
                    <strong>429</strong>
                    <span>{t('auth.hero.metrics.protection', { defaultValue: heroFallbacks.protection })}</span>
                  </div>
                </div>
              </div>
            </div>

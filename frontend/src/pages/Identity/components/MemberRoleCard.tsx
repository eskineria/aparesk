import type { RoleListDto } from '@/types/accessControl'
import {
    Card,
    CardBody,
    Dropdown,
...
import { TbCheck, TbClock, TbDotsVertical, TbEdit, TbShieldLock, TbTrash, TbUserShield } from 'react-icons/tb'
...
const MemberRoleCard = ({ role, onEdit, onDelete }: Props) => {
    const { t } = useTranslation()
    const { name, permissions, userCount } = role

    // Determine visuals based on role
    const Icon = name === 'Admin' ? TbShieldLock : TbUserShield
    const description = name === 'Admin'
        ? t('identity.full_access')
        : t('identity.access_limit', { count: permissions.length })

    // Show top 3 permissions as features
    const displayPermissions = permissions.slice(0, 3)

    return (
        <Card className="h-100">
            <div className="position-absolute top-0 end-0" style={{ width: 180, overflow: 'hidden' }}>
            ...
            <CardBody className="d-flex flex-column justify-content-between">
                <div className="d-flex mb-4">
                    <div className="flex-shrink-0">
                        ...
                        >
                            <Icon className="fs-1 text-primary" size={24} />
                        </div>
                    </div>
                    <div className="ms-3">
                    ...
                    <div className="ms-auto" style={{ zIndex: 2 }}>
                        <Dropdown align="end">
                            <DropdownToggle as="a" className="text-muted fs-xl drop-arrow-none cursor-pointer">
                            ...
                            <DropdownMenu>
                                <DropdownItem onClick={() => onEdit(role)}>
                                    ...
                                    {t('identity.edit_permissions')}
                                </DropdownItem>
                                {name !== 'Admin' && (
                                    <DropdownItem className="text-danger" onClick={() => onDelete(name)}>
                ...
                <ul className="list-unstyled mb-3">
                    {displayPermissions.map((feature, idx) => (
                        <li className={`d-flex align-items-center ${idx !== displayPermissions.length - 1 ? 'mb-2' : ''}`}
                            key={idx}>
                            <TbCheck className="fs-lg text-success me-2" /> {feature}
                        </li>
                    ))}
                    {permissions.length > 3 && (
                        <li className="d-flex align-items-center mt-2 text-muted fst-italic">
                            {t('identity.and_more', { count: permissions.length - 3 })}
                        </li>
                    )}
                </ul>
                <p className="mb-2 text-muted fw-semibold">{t('identity.total_users', { count: userCount })}</p>
                ...
                <div className="d-flex justify-content-between align-items-center mt-2">
                    ...
                    <Link to="#" onClick={() => onEdit(role)} className="btn btn-sm btn-outline-primary rounded-pill">
                        {t('common.details')}
                    </Link>
                </div>
            </CardBody>

...
import { LuPlus, LuTrash2, LuShieldCheck } from 'react-icons/lu'
                    ...
                    <Card className="border-0 shadow-sm">
                        <CardHeader className="bg-white border-bottom-0 pt-4 px-4 d-flex align-items-center justify-content-between">
                            <h4 className="mb-0 fw-bold d-flex align-items-center gap-2">
                                <LuShieldCheck className="text-warning" /> Denetim Kurulu Üyeleri
                            </h4>
                            <div className="d-flex gap-2">
                                <Button variant="outline-warning" size="sm" onClick={addMember} className="d-flex align-items-center gap-1">
                                    <LuPlus /> Üye Ekle
                                </Button>
                                <Button variant="warning" size="sm" onClick={save}>Değişiklikleri Kaydet</Button>
                            </div>
                        </CardHeader>
                        <Card.Body className="p-0 mt-2">
                            <div className="table-responsive">
                                <Table hover className="table-centered mb-0">
                                    ...
                                    <tbody>
                                        {assemblyDetail.boardMembers.filter((m: any) => m.boardType === BoardType.Audit).length === 0 ? (
                                            <tr><td colSpan={4} className="text-center py-5 text-muted">Henüz denetim kurulu üyesi eklenmemiş.</td></tr>
                                        ) : assemblyDetail.boardMembers.map((member: any, idx: number) => {
                                            if (member.boardType !== BoardType.Audit) return null;
                                            return (
                                                <tr key={idx}>
                                                    <td className="ps-4">
                                                        <Form.Select 

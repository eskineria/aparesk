...
const MessageDropdown = () => {
    ...
    return (
        <div className="topbar-item">
            <Dropdown align="end">
                <DropdownToggle as={'button'} className="topbar-link dropdown-toggle drop-arrow-none border-0 bg-transparent">
                    <LuBell className="fs-xxl" />
                    <span className="badge text-bg-danger badge-circle topbar-badge">7</span>
                </DropdownToggle>

                <DropdownMenu className="p-0 dropdown-menu-end dropdown-menu-lg shadow-lg border-0 mt-2">
                    <div className="px-3 py-2 border-bottom">
                        <Row className="align-items-center">
                            <Col>
                                <h6 className="m-0 fs-md fw-semibold">{t('topbar.notifications')}</h6>
                            </Col>
                            <Col className="text-end">
                                <Link to="" className="badge badge-soft-success badge-label py-1">
                                    {t('topbar.notifications_count', { count: 7 })}
                                </Link>
                            </Col>
                        </Row>
                    </div>
                                ...
                                <span className="d-flex gap-3 align-items-center">
                                    {message.user.icon && (
                                        <span className="avatar-md flex-shrink-0 position-relative">
                                            <span className={`avatar-title rounded-circle fs-22 ${message.user.bgClass} d-flex align-items-center justify-content-center`} style={{ width: '36px', height: '36px' }}>
                                                <message.user.icon className={`fs-4 text-${message.badge.variant}`} />
                                            </span>
                                            <span className={`position-absolute rounded-pill bg-${message.badge.variant} notification-badge`}>
                                                <message.badge.icon className="align-middle" size={10}></message.badge.icon>
                                            </span>
                                        </span>
                                    )}
                                    {message.user.avatar && (
                                        <span className="flex-shrink-0 position-relative">
                                            <img src={message.user.avatar} height={36} width={36} className="avatar-md rounded-circle" alt="" />
                                            <span className={`position-absolute rounded-pill bg-${message.badge.variant} notification-badge`}>
                                                <message.badge.icon className="align-middle" size={10}></message.badge.icon>
                                            </span>
                                        </span>

...
public class LocalStorageService : IStorageService
{
    ...
    private readonly IStringLocalizer<LocalStorageService> _localizer;

    public LocalStorageService(StorageOptions options, FileSecurityProvider security, IStringLocalizer<LocalStorageService> localizer)
    {
        _options = options;
        _security = security;
    ...
    public Task<Stream> DownloadAsync(string fileName, string folder = "")
    {
        ...
        var filePath = BuildPhysicalFilePath(sanitizedFolder, sanitizedName);
        if (!File.Exists(filePath))
            throw new FileNotFoundException(_localizer["FileNotFoundInStorage", sanitizedName].Value, sanitizedName);

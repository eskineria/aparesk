...
namespace Aparesk.Eskineria.Core.Notifications.Templates.Services;
...
public sealed class EmailTemplateSeedService
{
    ...
    public async Task SeedStartupAsync()
    {
        ...
        {
            ["VerifyEmailCode"] = new(StringComparer.OrdinalIgnoreCase) { "Your verification code", "Your Aparesk.Eskineria verification code", "Aparesk.Eskineria doğrulama kodunuz" },
            ["ResetPassword"] = new(StringComparer.OrdinalIgnoreCase) { "Password Reset Request", "Şifre Sıfırlama İsteği" },
            ["Welcome"] = new(StringComparer.OrdinalIgnoreCase) { "Welcome to Aparesk.Eskineria!", "Aparesk.Eskineria'ya Hoş Geldiniz!", "Aparesk.Eskineria'ya hoş geldiniz!" },

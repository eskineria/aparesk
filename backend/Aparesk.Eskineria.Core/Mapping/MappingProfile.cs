...
namespace Aparesk.Eskineria.Core.Mapping;
        ...
        _mappingTypes = assembly.GetExportedTypes()
            .Where(t =>
                ...
                t.GetInterfaces().Any(i =>
                    i.IsGenericType && i.GetGenericTypeDefinition() == typeof(Aparesk.Eskineria.Core.Mapping.Abstractions.IMapFrom<>)))
            .ToList();
    }
    ...
    public void Apply(TypeAdapterConfig config)
    {
        ...
        foreach (var type in _mappingTypes)
        {
            ...
            var methodInfo = type.GetMethod(
                                 nameof(Aparesk.Eskineria.Core.Mapping.Abstractions.IMapFrom<object>.Mapping),
                                 BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                                 binder: null,
                                 types: new[] { typeof(TypeAdapterConfig) },
                                 modifiers: null)
                             ?? type.GetInterfaces()
                                 .Where(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(Aparesk.Eskineria.Core.Mapping.Abstractions.IMapFrom<>))
                                 .Select(i => i.GetMethod(nameof(Aparesk.Eskineria.Core.Mapping.Abstractions.IMapFrom<object>.Mapping)))
